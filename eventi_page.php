        ...
        if($eventi != null){
            ...
            foreach($eventi as $evento){
                ...
                <img class="eventImg" src="img/' . $evento['url_immagine'] . '" alt="Immagine panoramica della citt&agrave; di '. basename($evento['url_immagine'], ".jpg"). '"/>
                    ...
                    if($checkDate[$index]){
                        $listaEventi.= '<div class="notAvailable">EVENTO TRASCORSO</div></dd>'; 
                    }
                    else {
                        $listaEventi .= '<a tabindex="0" aria-label="Compra il biglietto per la fiera di ' . basename($evento['url_immagine'], ".jpg") . '" class="eventButton" href="buy_Ticket.php?ID='.$evento['id_Evento'].'">COMPRA BIGLIETTO</a> </dd>';
                    }
                $index++;
            }
