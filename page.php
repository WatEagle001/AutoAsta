...
class page {
    ...
    private static function checkFileName($name){
        return ($_SERVER['SCRIPT_NAME'] == "/msold/". $name);
    }
    ...
    public function updateUserInfo($post, utente_Registrato $utente,$file){
        ...
        if($post['psw'] == $post['password-repeat']){
            $utente->updateUserInfo($post['username'],$post['psw'],$url_img,$post['data_nascita']);
            if($utente){
                return true;
            }
        }
    ...
    public function printBreadcrumb(){
        if($this->checkFileName("index.php")){
            echo "<p aria-label='ti trovi in home' tabindex='0' lang=\"en\">Home</p>";
        }
        else{
        ...
        else if($this->checkFileName("termini-condizioni.php")){
            echo"Termini e Condizioni</p>";
        }
